...
knitr::opts_knit$set(root.dir = '~r_programming/cbds')
```
...
n_distinct(pets$animal_s_name, na.rm = TRUE)
...
pets %>% 
  ...
  arrange(-n) %>%
  top_n(n = 10) %>%
  knitr::kable(., caption = "Top 10 Cat Names in Seattle", format = "html")
```
...
pets %>% filter(species %in% c("Cat", "Dog"), animal_s_name != "") %>%
         group_by(species, animal_s_name) %>% 
         summarise(N = n()) %>% 
         top_n(10, N) %>% 
         ggplot(aes(animal_s_name, N, fill = species)) + geom_bar(stat = "identity") + 
